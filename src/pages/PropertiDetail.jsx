...
import { propertyAPI, getImageUrl, settingsAPI, categoryAPI } from '../utils/api';
...
const PropertiDetail = () => {
  ...
  const loadData = async () => {
    try {
      const [propertyRes, settingsRes, categoriesRes] = await Promise.all([
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          {categories.length > 0 && (
            <div className="mt-12 mb-8" data-aos="fade-up">
              <h2 className="text-3xl font-bold mb-6 text-blue-600">Kategori Properti</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {categories.map((category) => (
                  <Link
                    key={category.id}
                    to={`/properti?category=${category.id}`}
                    className="bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:-translate-y-2 border-2 border-blue-100 hover:border-blue-300"
                  >
